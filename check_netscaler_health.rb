...
case options[:mode]
when 'cpuusage'
  system_health = JSON.parse(
    RestClient.get(
      "https://#{options[:host]}/nitro/v1/stat/systemcpu",
      accept: :json,
...
when 'memusage'
  system_health = JSON.parse(
    RestClient.get(
      "https://#{options[:host]}/nitro/v1/stat/systemmemory",
      accept: :json,
      cookies: { 'NITRO_AUTH_TOKEN' => login.cookies['NITRO_AUTH_TOKEN'] }
    )
  )
...
when 'hastate'
  ...
  if system_health['hanode']['hacurstatus'] == 'YES'
    is_critical = true if system_health['hanode']['hacurstate'] != 'UP'
