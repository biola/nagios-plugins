...
login = RestClient.post(
  "https://#{options[:host]}/nitro/v1/config/login/",
  login_payload.to_json,
  content_type: 'application/vnd.com.citrix.netscaler.login+json',
  accept: :json
)

# Get the HA status of the node
ha_status = JSON.parse(
  RestClient.get(
    "https://#{options[:host]}/nitro/v1/stat/hanode",
    accept: :json,
    cookies: { 'NITRO_AUTH_TOKEN' => login.cookies['NITRO_AUTH_TOKEN'] }
  )
)

if ha_status['hanode']['hacurmasterstate'] == 'Primary'
  # Get the vserver status
  vserver_status = JSON.parse(
    RestClient.get(
      "https://#{options[:host]}/nitro/v1/stat/lbvserver/#{options[:vserver]}",
      content_type: 'application/vnd.com.citrix.netscaler.lbvserver+json',
      accept: :json,
      cookies: { 'NITRO_AUTH_TOKEN' => login.cookies['NITRO_AUTH_TOKEN'] }
    )
  )
  ...
  # Log out
  RestClient.post(
    "https://#{options[:host]}/nitro/v1/config/logout/",
    { 'logout' => {} }.to_json,
    content_type: 'application/vnd.com.citrix.netscaler.logout+json',
    accept: :json,
    cookies: { 'NITRO_AUTH_TOKEN' => login.cookies['NITRO_AUTH_TOKEN'] }
  )
  ...
  puts 'OK: node is secondary, vserver check skipped'
  exit 0
end
